...
from verify_cartridge import C                           # noqa: E402
from verify_conformal import fetch                       # noqa: E402
...
def main():
    M = fetch()
    vv = {k: float(np.abs((np.cross(M[k][:, 0], M[k][:, 1]) * M[k][:, 2]).sum() / 6))
          for k in M}
    HOUSE = M[max(vv, key=vv.get)]
    CAR = M[min(vv, key=vv.get)]
    ...
    axes[0].legend(handles=[
        ...
        Patch(fc="#2b6cb0", label="NEW  BOTTOM_CARRIER"),
        Patch(fc="#c9a227", alpha=0.55, label="NEW  CONFORMAL_HOUSING")],
        loc="upper left", fontsize=9)
    fig.suptitle("OneGrip Play — 스톡 짐벌 컨포멀 매립 하우징 (20 deg, "
                 "GRIP_AXIS perp DECK = 90.000000 deg)", fontsize=13)
    fig.tight_layout()
    out = os.path.join(sg.OUT, "shot_conformal.png")
