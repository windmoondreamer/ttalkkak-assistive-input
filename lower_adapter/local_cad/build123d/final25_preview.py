...
def main():
    house=PR.read_stl(os.path.join(STL,"ERGO_HOUSING_25_WRAP_FINAL.stl"))
    ...
    for k,wd in VIEWS.items():
        draw(f"FINAL25_{k}",wd,f"OneGrip Play — 25deg + WRAP SKIRT FINAL  [{k}]")
    draw("FINAL25_CUTAWAY",np.array([-0.75,-0.62,0.23]),
