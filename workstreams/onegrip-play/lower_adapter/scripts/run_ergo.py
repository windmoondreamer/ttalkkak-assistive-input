"""W2 인체공학 외피 FS 실행기 (신규 Part Studio 전용)."""
...
FS_PATH = os.path.join(ROOT, "lower_adapter", "cad", "OneGrip_Ergo.fs")
TARGET = os.path.join(OUT, "ergo_target.json")

VARS = [("wrist_pad_angle", "7 deg", "ANGLE"),
        ("wrist_pad_length", "85 mm", "LENGTH"),
        ("wrist_pad_width", "86 mm", "LENGTH"),
        ("wrist_pad_radius", "16 mm", "LENGTH"),
        ("wrist_anchor_y", "-20 mm", "LENGTH"),
        ("front_rake", "5 deg", "ANGLE"),
        ("knee_angle", "34 deg", "ANGLE"),
        ("knee_y", "18 mm", "LENGTH"),
        ("pocket_clearance2", "0.3 mm", "LENGTH"),
        ("edge_fillet", "6 mm", "LENGTH"),
        ("trace_steps", "0 mm", "LENGTH")]

FORBIDDEN = {wc.EID_JOYSTICK, "df9a32f1f239bc71a732f5d3",
             "2e024442c796323fd37e49c3", "bbfebe9c42748fb6d5b912e8",
             "f698b10ce216ca7c95051dd3", "8945f7ac4100dfd52a8c8dba"}
...
def init():
    ps = wc.create_part_studio("OneGrip_ErgoShell")
    fs = wc.create_feature_studio("OneGrip_Ergo_FS")
    save({"ps_eid": ps["id"], "fs_eid": fs["id"]})
    print("Part Studio   ", ps["id"], ps.get("name"))
    print("Feature Studio", fs["id"], fs.get("name"))


def post(feature, eid):
    guard(eid); wc._guard(wc.DID, wc.WID)
...
def make_vars():
    local_var_template()
    t = target(); ids = t.get("varIds", {})
    for n, v, ty in VARS:
        fid, st = post(wc.assign_variable(n, v, ty), t["ps_eid"])
        ids[n] = fid
        print(f"  #{n:<20s} = {v:<9s} {st}")
    t["varIds"] = ids; save(t)


def setvar(name, expr, ty="LENGTH"):
    local_var_template()
    t = target(); fid = t["varIds"][name]
    f = wc.assign_variable(name, expr, ty); f["featureId"] = fid
    r = session().post(f"{BASE}/partstudios/d/{wc.DID}/w/{wc.WID}/e/{t['ps_eid']}"
...
def upload():
    ...
    spec = session().get(f"{BASE}/featurestudios/d/{wc.DID}/w/{wc.WID}/e/{t['fs_eid']}"
                         "/featurespecs", timeout=180).json()
...
def custom(stage, name, ns, ft):
    return {"btType": "BTMFeature-134", "namespace": ns, "name": name,
            ...
            "parameters": [{"btType": "BTMParameterEnum-145", "namespace": ns,
                            "enumName": "ErgoStage", "value": stage,
                            "parameterId": "stage"}]}


def add(stage, name):
    t = target(); ns, ft = t.get("namespace"), t.get("featureType")
    if ns is None:
        ns, ft = upload()
    fid, st = post(custom(stage, name, ns, ft), t["ps_eid"])
    print(f"  {name:<26s} post={st}  featureId={fid}")
    t.setdefault("stageIds", {})[stage] = fid; save(t)
...
def parts():
    ...
    if r.status_code != 200:
        print("parts GET", r.status_code); return
    p = r.json(); sol = [x for x in p if x.get("bodyType") == "solid"]
    print(f"  records {len(p)}  solid {len(sol)}  wire {len(p)-len(sol)}")
    for x in sol:
        m = session().get(f"{BASE}/parts/d/{wc.DID}/w/{wc.WID}/e/{t['ps_eid']}/partid/"
                          f"{x['partId'].replace('/', '%2F')}/massproperties",
                          params=[("configuration", "default")], timeout=180)
        v = ""
        if m.status_code == 200:
            b = m.json()["bodies"]; v = f"{b[list(b)[0]]['volume'][0]*1e9:11.1f} mm3"
...
if __name__ == "__main__":
    c = sys.argv[1]
    if c == "init": init()
    elif c == "vars": make_vars()
    elif c == "upload": print("ns/ft:", upload())
    elif c == "add": add(sys.argv[2], sys.argv[3])
    elif c == "setvar": setvar(sys.argv[2], sys.argv[3], sys.argv[4] if len(sys.argv)>4 else "LENGTH")
    elif c == "parts": parts()
