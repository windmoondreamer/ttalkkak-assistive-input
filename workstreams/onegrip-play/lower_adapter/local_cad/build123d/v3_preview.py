...
def main():
    house=PR.read_stl(os.path.join(STL,"ERGO_HOUSING_25_WRAP_FINAL_V3.stl"))
    ...
    for k,wd in VIEWS.items():
        draw(f"V3_{k}",wd,f"OneGrip Play — 25deg + WRAP + 팔받침 133.6mm + 접합 블렌드  [{k}]")
    draw("V3_CUTAWAY",np.array([-0.75,-0.62,0.23]),
