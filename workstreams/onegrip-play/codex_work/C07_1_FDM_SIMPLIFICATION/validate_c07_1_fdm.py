...
OUT = WORK / "validation"
STEP = WORK / "outputs" / "C07_1_SOURCE_FAITHFUL_THUMB_CORE_SIMPLIFIED.step"
STL = WORK / "outputs" / "C07_1_SOURCE_FAITHFUL_THUMB_CORE_SIMPLIFIED.stl"
BUILDER = WORK / "c07_1_builder.py"
META = WORK / "outputs" / "c07_1_build.json"
...
def render_mesh(Tl, support_ids, bed_ids, path: Path, underside=False):
    ...
    title = "C07.1 underside: orange = >45 deg support surfaces" if underside else "C07.1 JOY_AXIS_UP: blue = true bed contact, orange = support surfaces"
...
def render_slots(seat_rows, path: Path):
    ...
    d = ImageDraw.Draw(im)
    d.text((20, 15), "C07.1 actual STEP sampling: 8 switch seats / 16 terminal slots", fill=(20, 20, 20))
...
def main():
    ...
    # Final visual deliverables are the eight same-camera comparison renders in
    # ../renders.  Keep this validator machine-readable and avoid duplicating
    # diagnostic PNGs with a different camera convention.
    ...
    (OUT / "C07_1_FDM_VALIDATION.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
    print(json.dumps({
        "step": step_info,
        "stl": stl_info,
