"""I10 - C07, C06 rebuilt for FDM manufacturability (REV_I FDM cleanup).
...
def main() -> int:
    ...
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            ...
            br = pbox(mid, d, BRIDGE_W, slab_bot, top, th, "BR")
            body = (body + br).clean()
    ...
    # ---- 4 vertical deck-to-slab walls ----------------------------------
    body = (body + deck).clean()
    nwall = 0
    for s in ("T7", "T8", "T4", "T6", "T2"):
        a = seats[s]["top"]
        rel = np.asarray(a, float) - jc
        ...
        rr = float(np.interp(th % (2 * np.pi), np.asarray(ang), rad, period=2 * np.pi))
        r_in = max(DECK_APERTURE + 1.5, rr - 4.0)
        r_out = min(nrm, rr - 0.5)
        if r_out - r_in < 3.0:
            continue
        mid = jc + u * (0.5 * (r_in + r_out))
        # a wall whose faces are parallel to PRINT_UP: self-supporting all the
        # way up from the deck.  C06 used inclined links whose undersides were
        # 58 deg overhangs.
        wall = pbox(mid, r_out - r_in, WALL_T, deck_top - DECK_T, slab_bot + 0.6, th, "WALL_" + s)
        body = (body + wall).clean()
        nwall += 1
    ...
    # ---- 6 chamfered standoff pads --------------------------------------
    from scipy.spatial import cKDTree
    Psh = np.concatenate([L.surf_points(s, 900000, tol=0.06) for s in shell])
    Psh = Psh[np.linalg.norm(Psh - L.DATUM_P, axis=1) < 55.0]
    Pc = L.surf_points(body, 400000, tol=0.06)
    tree = cKDTree(Psh)
    dist, idx = tree.query(Pc)
    print("6 core-to-shell gap  min %.3f  p05 %.3f  p50 %.3f"
          % (dist.min(), np.percentile(dist, 5), np.percentile(dist, 50)))
    order = np.argsort(dist)
    chosen, added, below = [], 0, 0
    for i in order:
        if dist[i] > 6.0 or added >= 6:
            break
        p = Pc[i]
        # A pad that dips under the deck becomes the lowest point of the part
        # and steals the first layer: bed contact then reads 0.0 mm2 on a part
        # with a flat 1100 mm2 underside.
        if zh(p) < deck_bot + 1.2:
            below += 1
            continue
        if any(np.linalg.norm(p - q) < 12.0 for q in chosen):
            continue
        q = Psh[idx[i]]
        ax = q - p
        ...
        ax = ax / n
        h = n - SHELL_CLR
        # BOTH ends must stay above the bed.  Filtering only the contact point
        # still let a downward-pointing pad hang 5.25 mm under the deck.
        if min(zh(p - ax * PAD_CHAMFER), zh(p + ax * h)) < deck_bot:
            below += 1
            continue
        a2 = np.array([1.0, 0.0, 0.0]) if abs(ax[0]) < 0.9 else np.array([0.0, 1.0, 0.0])
        e1 = L.unit(np.cross(a2, ax))
        # truncated pyramid: the flank is 45 deg or shallower, so the pad is
        # self-supporting whichever way it faces.  C06 used square pads whose
        # sides were vertical cliffs off the slab.
        base = Plane(origin=L.xyz(p - ax * PAD_CHAMFER), x_dir=L.xyz(e1), z_dir=L.xyz(ax)) * Box(
            PAD_XY + 2 * PAD_CHAMFER, PAD_XY + 2 * PAD_CHAMFER, PAD_CHAMFER + h,
            align=(Align.CENTER, Align.CENTER, Align.MIN))
        cone = Plane(origin=L.xyz(p), x_dir=L.xyz(e1), z_dir=L.xyz(ax)) * Box(
            PAD_XY, PAD_XY, h, align=(Align.CENTER, Align.CENTER, Align.MIN))
        trial = solid((body + base + cone).clean(), "CORE")
        if len(list(trial.solids())) == 1 and all(L.inter_vol(trial, sh) < 1.0e-6
                                                  for sh in shell):
            body = trial
            added += 1
        chosen.append(p)
    print("   standoff pads added %d (%d candidates skipped for dipping below the bed)"
          % (added, below))
    L.memory("pads")
    if stage < 7:
        return 0
    ...
    body = solid(body, "CORE")
    body.label = "C07_SOURCE_FAITHFUL_THUMB_CORE_REFINED"
    print("")
    print("C07: vol %.3f  faces %d  solids %d  valid %s"
          % (L.vol(body), len(list(body.faces())), len(list(body.solids())),
             bool(body.is_valid)))
    export_step(body, str(OUT / "C07_SOURCE_FAITHFUL_THUMB_CORE_REFINED.step"))
    export_stl(body, str(OUT / "C07_SOURCE_FAITHFUL_THUMB_CORE_REFINED.stl"),
               tolerance=0.03, angular_tolerance=0.10)

    L.write_json(OUT / "i10_c07.json",
                 {"printUpWorld": jw.tolist(), "printOrigin": jc.tolist(),
                  ...
                  "standoffs": added, "standoffsSkippedBelowBed": below, "padXY": PAD_XY, "padChamferMm": PAD_CHAMFER,
