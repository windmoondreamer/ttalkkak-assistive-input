...
Copied again so that LAB resolves to REV_L_C09_CONTINUOUS_CURVED_CARRIER.
REV_A..REV_H are immutable.

READ-ONLY outside thumb_inner_housing_lab/REV_L_C09_CONTINUOUS_CURVED_CARRIER.  Never imports a production
...
LAB = Path(__file__).resolve().parents[1]          # -> REV_L_C09_CONTINUOUS_CURVED_CARRIER
REV_A = LAB.parent / "REV_A"                       # read-only reference
REV_B = LAB.parent / "REV_B"                       # read-only reference
REV_C = LAB.parent / "REV_C"                       # read-only reference
REV_D = LAB.parent / "REV_D_EXACT_SOURCE"          # read-only reference
REV_E = LAB.parent / "REV_E_C02_REFINED"           # read-only reference
REV_F = LAB.parent / "REV_F_RIGID_FIT"             # read-only reference
REV_G = LAB.parent / "REV_G_BACKPLATE_DRIVEN_FREEZE"  # read-only reference
REV_H = LAB.parent / "REV_H_ORIGINAL_SWITCH_MECHANISM"  # READ-ONLY FOUNDATION
REV_I = LAB.parent / "REV_I_SOURCE_FAITHFUL_THUMB_PROTOTYPE"  # READ-ONLY, holds C07
REV_J = LAB.parent / "REV_J_DOCS101_REVALIDATION"  # READ-ONLY, docs/101 revalidation
REV_K = LAB.parent / "REV_K_C08_LOCAL_CONFORMAL_RECEIVER"  # READ-ONLY, C08
CODEX_C07_1 = ROOT / "codex_work/C07_1_FDM_SIMPLIFICATION"  # READ-ONLY, C07.1
EXACT = ROOT / "thumb_exact_onshape_source"        # NEW MASTER AUTHORITY

# --- authoritative source paths (read-only) --------------------------------
SRC = {
    # NEW MASTER AUTHORITY: exact Onshape export, version THUMB_LOWER15_HOUSING_V1
    "JAD_APPROVED": ROOT / "thumb_exact_onshape_source/JaD_THUMB_LOWER15_APPROVED.step",
    "JFD_APPROVED": ROOT / "thumb_exact_onshape_source/JfD_THUMB_LOWER15_APPROVED.step",
    # LEGACY DEFECTIVE LOCAL RECONSTRUCTION -- historical explanation only,
    # never a geometry authority.
    ...
    # docs/101 frozen Finger shell -- NEW integration authority
    "D101_JAD": ROOT / "build123d_workbench/out/direct_embedded_finger_switch_final_candidate/ONEGRIP_DIRECT_EMBEDDED_JaD.step",
    "D101_JFD": ROOT / "build123d_workbench/out/direct_embedded_finger_switch_final_candidate/ONEGRIP_DIRECT_EMBEDDED_JfD.step",
    "D101_SWITCHES": ROOT / "build123d_workbench/out/direct_embedded_finger_switch_final_candidate/ALL8_DETAILED_SWITCH_PLACEMENT.step",
    "D101_BASE_JAD": ROOT / "build123d_workbench/out/lower15_true_bare_finger_base/LOWER15_TRUE_BARE_FINGER_JaD.step",
    "D101_BASE_JFD": ROOT / "build123d_workbench/out/lower15_true_bare_finger_base/LOWER15_TRUE_BARE_FINGER_JfD.step",
    "C07_1": ROOT / "codex_work/C07_1_FDM_SIMPLIFICATION/outputs/C07_1_SOURCE_FAITHFUL_THUMB_CORE_SIMPLIFIED.step",
