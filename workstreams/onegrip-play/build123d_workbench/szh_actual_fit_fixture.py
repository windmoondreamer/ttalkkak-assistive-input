...
def foundation_frame() -> tuple[Shape, list[Shape]]:
    ...
    # Direction ticks stay clear of the shaft/gimbal sweep, but they must also
    # sit on solid ring material.  Reaching inward from the aperture edge left
    # them as 4 mm cantilevers held by a 0.44 mm2 butt joint with nothing
    # underneath, so each tick now lies wholly on the band beside its label.
    tick_z = FRAME_Z0 + FRAME_THICKNESS
    ticks = [
        local_box((24.7, 4.4, tick_z), (28.7, 5.2, tick_z + 0.55), "TILT_MARK_PLUS_X"),
        local_box((-28.7, 4.4, tick_z), (-24.7, 5.2, tick_z + 0.55), "TILT_MARK_MINUS_X"),
        local_box((-14.5, 17.0, tick_z), (-13.7, 21.0, tick_z + 0.55), "TILT_MARK_PLUS_Y"),
        local_box((-14.5, -21.0, tick_z), (-13.7, -17.0, tick_z + 0.55), "TILT_MARK_MINUS_Y"),
    ]
    ...
    labels = [
        ...
        # -7.5 rather than -5.0: Text centres on its origin, so at -5.0 the
        # glyphs ran into JOYSTICK AXIS (0.0805 mm3 of shared solid).
        text_solid("TEST ONLY", 1.55, -7.5, 19.0, tick_z),
    ]
